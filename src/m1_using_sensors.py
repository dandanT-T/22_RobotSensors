...
def run_test_beep_and_tone():
    ...
    t = ToneMaker()
    for k in range(10):
        t.tone(300*(k+1),100).wait()

    # -------------------------------------------------------------------------
    # DO: 4.  Implement and test this method.
    # -------------------------------------------------------------------------
    # IMPORTANT:
...
class DriveSystem(object):
    ...
    def go_straight_for_inches_using_sensor(self, inches, speed):
        # self.go(speed,speed)
        if InfraredProximitySensor.get_distance(inches) <= abs(inches):
        #     self.stop()
        while True:
            self.go(speed,speed)
            if InfraredProximitySensor(inches) <= abs(inches):
                self.stop()
                break
        pass
